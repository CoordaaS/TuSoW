...
public abstract class AbstractRepresentation implements Representation {
    ...
    protected static final String writeAsXML(Object thiz) {
        final StringWriter writer = new StringWriter();
        try {
            xmlMapper.writeValue(writer, thiz);
            return writer.toString();
        } catch (IOException e) {
            throw new IllegalStateException(e);
        }
    ...
    protected static <R> R fromJSON(String representation, Class<R> type) throws IOException {
        return jsonMapper.readValue(representation, type);
    }

    protected static <R> R fromYAML(String representation, Class<R> type) throws IOException {
        return yamlMapper.readValue(representation, type);
    }

    protected static <R> R fromXML(String representation, Class<R> type) throws IOException {
        return xmlMapper.readValue(representation, type);
    }

    protected static <R> R parse(String mimeType, String representation, Class<R> type) throws IOException {
