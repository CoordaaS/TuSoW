...
abstract class TestTupleSpaceInspectability<T : Tuple, TT : Template, K, V, M : Match<T, TT, K, V>, TS : InspectableTupleSpace<T, TT, K, V, M>>(tupleTemplateFactory: TupleTemplateFactory<T, TT, K, V, M>) : TestBaseLinda<T, TT, K, V, M>(tupleTemplateFactory) {
    ...
    fun testGet() {
        ...
        assertEquals(tuples.size * 2 + 4, observableBehaviour.size.toLong())
        assertEquals(
    ...
    fun testTryAbsentFail() {
        ...
        assertEquals(6, observableBehaviour.size.toLong())
        assertEquals(
                listOf(expectedEvent1, expectedEvent2, expectedEvent3, expectedEvent4, expectedEvent5, expectedEvent6),
                observableBehaviour
        )
